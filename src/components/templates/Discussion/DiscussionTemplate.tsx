...
interface DiscussionTemplateProps {
  rfqId: string;
}

const DiscussionTemplate: React.FC<DiscussionTemplateProps> = ({ rfqId }) => {
  ...
  const {
    data: {
      payload: { data: discussion = [], last_page: lastPage = 1 } = {},
    } = {},
    isLoading,
  ...
  useEffect(() => {
    if (page === 1) {
      ...
      setMessages((prevMessages) => [
        ...
        ...discussion.filter(
          (msg: any) =>
            !prevMessages.some((prevMsg: any) => prevMsg.id === msg.id)
        ),
      ]);
    }
  ...
  return (
    <div
      className="max-w-3xl mx-auto p-4 h-screen"
